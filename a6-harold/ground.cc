                           ...
                           const std::vector<Point2> &stroke2d)
{
    ...
    ScreenPtHitsGround(view_matrix, proj_matrix, stroke2d.front(), &startP);
    ScreenPtHitsGround(view_matrix, proj_matrix, stroke2d.back(), &endP);

    Vector3 v = startP - endP;
    Vector3 up = Vector3(0,1,0);
    Vector3 normal = v.Cross(up).ToUnit();
    ...
    for(int i=0; i < stroke2d.size();i++){
      ...
      float t;
      Point3 p;
      ray.IntersectPlane(startP,normal,&t,&p);
      stroke_to_3d.push_back(p);
    }
    ...
    for (int i=0; i<ground_mesh_.num_vertices(); i++) {
        ...
        Point3 closetP = P.ClosestPointOnPlane(startP,normal);
        float h = hfunc(normal,stroke_to_3d,closetP);

        if(h != 0.0){
          float d = P.DistanceToPlane(startP,normal);
          float w = fmax(0.0,1-pow(d/5,2));
          P = Point3(P.x(),(1.0 - w) * P.y() + w * h,P.z());
        }
